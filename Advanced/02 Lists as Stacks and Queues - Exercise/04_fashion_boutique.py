...
# •	If the sum becomes equal to the capacity of the current rack you
# must take a new one for the next clothes (if there are any left in the box).
...
for x in range(len(integers)):
    numbers.append(int(integers.pop()))

while numbers:
    if rack + numbers[-1] <= capacity:
        rack += numbers.pop()
    else:
        rack = 0
        total += 1
